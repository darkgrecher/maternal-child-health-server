...
export class AdminAnalyticsService {
  ...
  private formatLabel(date: Date) {
    return `${MONTH_LABELS[date.getMonth()]} ${date.getDate()}`;
  }

  private buildBuckets(start: Date, days: number) {
    const buckets: Date[] = [];
  ...
  private buildSeries(values: Date[], buckets: Date[]) {
  ...
  private buildTrend(current: number, previous: number): Trend {
    if (previous === 0) {
      return {
        current,
        previous,
  ...
  private normalizeRegion(value?: string | null) {
    const trimmed = value?.trim();
    return trimmed ? trimmed : 'Unassigned';
  }

  async getAnalytics(range?: string): Promise<AdminAnalyticsResponse> {
    ...
    const [
      ...
      newMidwivesPrevious,
      newChildrenPrevious,
      newPregnanciesPrevious,
      appointmentsPrevious,
      vaccinationsPrevious,
      newUsersRange,
      newMidwivesRange,
      newChildrenRange,
      newPregnanciesRange,
    ...
    const series: AnalyticsSeries = {
      labels,
      newUsers: this.buildSeries(
        newUsersRange.map((record) => record.createdAt),
        buckets,
      ),
      newChildren: this.buildSeries(
        newChildrenRange.map((record) => record.createdAt),
        buckets,
      ),
      newPregnancies: this.buildSeries(
        newPregnanciesRange.map((record) => record.createdAt),
        buckets,
      ),
      appointments: this.buildSeries(
        appointmentDates.map((record) => record.dateTime),
        buckets,
      ),
      vaccinations: this.buildSeries(
        vaccinationDates
    ...
    const vaccinationStatusStats: VaccinationStatusStat[] = vaccinationStatus.map((item) => ({
    ...
    const recentActivityItems: RecentActivity[] = recentActivities.map((activity) => ({
      id: activity.id,
      type: activity.type,
