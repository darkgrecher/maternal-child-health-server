...
export class DashboardService {
  ...
  private getChildScope(actor: ActorContext) {
    return actor.actorType === 'midwife'
      ? { midwifeId: actor.id }
      : { userId: actor.id };
  }

  private getPregnancyScope(actor: ActorContext) {
    return actor.actorType === 'midwife'
      ? { midwifeId: actor.id }
      : { userId: actor.id };
  }
  ...
  async getDashboard(actor: ActorContext): Promise<DashboardResponse> {
    const childScope = this.getChildScope(actor);
    ...
    const { items: overdueVaccinations, count: overdueVaccinationsCount } =
      await this.getOverdueVaccinations(actor);
